---
title: Chat
description: Render role-tagged messages into chat prompts and generate responses.
navigation:
...
[`BuiltinTemplate`] (or a custom Jinja2-subset template), then runs text
...
| Format | Token wrapping | Suggested for |
| --- | --- | --- |
| `ChatMl` (default) | `<tool_call>{...}</tool_call>` | Qwen 2.5, Hermes, generic ChatML |
| `Mistral` | `[TOOL_CALLS][{...}]` | Mistral / Mixtral |
| `Llama3` | `<|python_tag|>{...}` | Llama 3, Firefunction-v2 |
| `Plain` | (none, raw JSON) | Custom prompts |
| `Functionary` | `<|start|>function<|message|>...<|call|>` | Functionary models |
...
non-streaming use; `ToolCallStream` emits `ToolCallDelta { index, id,
name, arguments, completed }` chunks and is the right tool for SSE-style
streams.
