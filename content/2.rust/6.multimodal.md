---
title: Multimodal
description: Use mtmd vision and audio support with text models, projector files, and image or audio prompts.
navigation:
  icon: i-lucide-eye
seo:
  title: Multimodal
...
| Constructor | Use |
| --- | --- |
| `from_image_data(nx, ny, data)` | RGB8 pixel buffer, `nx * ny * 3` bytes. |
| `from_audio_data(data)` | Float audio buffer; the projector is responsible for resampling. |
| `from_file(path)` | Decode an image from disk (requires the `mtmd` feature, which pulls in the `image` crate). |
...
`n_threads = 1`). Pass `MtmdContext::init_from_file_with(path, model,
params)` to override:
