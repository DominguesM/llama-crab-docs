...
description: Generate text completions with prompts, options, streaming, and custom samplers.
navigation:
...
non-EOS tokens, the [`StopReason`] (`Length`, `Eos`, `Stop`, `ToolCalls`)
and optional per-token [`CompletionLogprobs`].
