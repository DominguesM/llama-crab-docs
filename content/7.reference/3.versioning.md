...
`0.1.6` (Rust crates and `@llama-crab/core` / `@llama-crab/tauri` are
released together).

Notable cross-package guarantees and pin-level facts:

- `Cargo.toml` workspace `version = "0.1.6"`. The same value drives the
  version of every workspace crate.
- The workspace dependency on `llama-crab-sys` is currently pinned to
  `0.1.5` even though the high-level `llama-crab` is at `0.1.6`. The
  public API is unchanged between those releases, so consumers should
  pin `llama-crab = "0.1.6"` and let Cargo resolve the FFI version
  transitively.
- `@llama-crab/core` and `@llama-crab/tauri` are at `0.1.6` and depend
