...
The workspace is at version `0.1.7` and **MSRV is 1.88** (pinned in
`rust-toolchain.toml`).
...
| `packages/core` | Shared TypeScript contracts and helpers (`@llama-crab/core` v0.1.7). |
| `packages/tauri` | TypeScript Tauri client (`@llama-crab/tauri` v0.1.7). |
...
For HF model resolution in 0.1.7, enable `hf-hub` as well:
...
mode. The wrapper is wired to the `llama-crab = "0.1.7"` published
